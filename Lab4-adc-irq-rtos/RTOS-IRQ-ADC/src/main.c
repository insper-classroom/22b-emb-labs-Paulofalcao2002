...
static void task_adc(void *pvParameters) {
  ...
  while (1) {
    if (xQueueReceive(xQueuePROC, &(adc), 1500)) {
      printf("ADC: %d \n", adc);
    } else {
      printf("Nao chegou um novo dado em 1.5 segundos\n");
    }
  }
}
...
static void task_proc(void *pvParameters) {
  ...
  while (1) {
    if (xQueueReceive(xQueueADC, &(adc), 1000)) {
      ...
      nDados++;
      if (nDados == 10) {
        xQueueSend(xQueuePROC, &adc, 1000);
        nDados = 0;
        media = 0;
      }
    } else {
      printf("Nao chegou um novo dado no proc\n");
    }
  }
}
