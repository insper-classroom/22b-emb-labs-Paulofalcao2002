...
void RTC_Handler(void) {
	...
	// o código para irq de segundo vem aqui
		// printf("Segundo\n");
		int id = 1;
		xQueueSendFromISR(xQueueRTC, &id, &xHigherPriorityTaskWoken);
    }
...
void lv_termostato(void) {
    ...
    lv_obj_add_event_cb(power, power_handler, LV_EVENT_ALL, NULL);
    lv_obj_align(power, LV_ALIGN_BOTTOM_LEFT, 10, -20);
	lv_obj_add_style(power, &style, 0);
    labelPower = lv_label_create(power);
    lv_label_set_text(labelPower, "[  " LV_SYMBOL_POWER);
    lv_obj_center(labelPower);

	// menu
    lv_obj_t * menu = lv_btn_create(lv_scr_act());
    lv_obj_add_event_cb(menu, menu_handler, LV_EVENT_ALL, NULL);
    lv_obj_align_to(menu, power, LV_ALIGN_RIGHT_MID, 30, -10);
	...
	// DOWN
    lv_obj_t * down = lv_btn_create(lv_scr_act());
    lv_obj_add_event_cb(down, down_handler, LV_EVENT_ALL, NULL);
	lv_obj_align(down, LV_ALIGN_BOTTOM_RIGHT, -10, -20);
	lv_obj_add_style(down, &style, 0);
    labelDown = lv_label_create(down);
    lv_label_set_text(labelDown, LV_SYMBOL_DOWN "  ]");
    lv_obj_center(labelDown);

	// UP
    lv_obj_t * up = lv_btn_create(lv_scr_act());
    lv_obj_add_event_cb(up, up_handler, LV_EVENT_ALL, NULL);
	lv_obj_align_to(up, down, LV_ALIGN_LEFT_MID, -70, -10);
    ...
    lv_obj_align(labelSetValue, LV_ALIGN_RIGHT_MID, -10 , -45);
...
static void task_RTC(void *pvParameters) {
	...
	for (;;)  {
		if (xQueueReceive(xQueueRTC, &id, 1)) {
			// printf("Segundo\n");
			pisca = 1;
		}
		if (pisca) {
...
int main(void) {
	...
	xQueueRTC = xQueueCreate(100, sizeof(int));
	if (xQueueRTC == NULL)
